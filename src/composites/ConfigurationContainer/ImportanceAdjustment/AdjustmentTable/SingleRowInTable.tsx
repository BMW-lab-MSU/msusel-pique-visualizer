...
const SingleTableRow: React.FC<SingleTableRowProps> = ({
    ...
    return (
        <Table.Row>
            ...
            <Table.Cell align='center' justify={'center'}>{(mode =="Evaluate") ? qualityAspectValue.toFixed(2)
            : <Box style={{ position: 'relative', padding: '20px' }}>
                    <Slider.Root
                        value={[qualityAspectValue]}
                    ...
                    <div style={{ position: 'absolute', top: '-2px', left: `${qualityAspectValue * 100}%`, transform: 'translateX(-50%)' }}>
                        {qualityAspectValue.toFixed(2)}
                    </div>
                </Box>
            }</Table.Cell>
